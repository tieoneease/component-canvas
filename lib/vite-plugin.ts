...
function loadPreviewModule(): string {
  return [
    ...
    '  clearMount();',
    '',
    '  const queryPresentationId = new URLSearchParams(window.location.search).get("presentation");',
    '  if (queryPresentationId) {',
    '    await renderPresentation(queryPresentationId, version);',
    '    return;',
    '  }',
    '',
    ...
    '  }',
    '',
    '  if (route.type === "variant") {',
    '    const resolved = resolveVariant(route.workflowId, route.variantId);',
    '',
    '    if (!resolved.component || !resolved.screen || !resolved.variant) {',
    '      renderMessage("Variant not found", resolved.message);',
    '      return;',
    '    }',
    '',
    '    const { component, screen, variant } = resolved;',
    '',
    '    document.title = variant.title ?? screen.title ?? screen.id;',
    ...
    '  }',
    '',
    '  if (route.type === "presentation") {',
    '    await renderPresentation(route.presentationId, version);',
    '    return;',
    '  }',
    '',
    ...
    '    "Use #/screen/<workflowId>/<screenId>, #/variant/<workflowId>/<variantId>, or #/render/<id>."',
    '  );',
    '}',
    '',
    ...
    '    return {',
    '      type: "variant",',
    '      workflowId: decodeURIComponent(variantMatch[1]),',
    '      variantId: decodeURIComponent(variantMatch[2])',
    '    };',
    '  }',
    '',
    ...
    '    return {',
    '      type: "presentation",',
    '      presentationId: decodeURIComponent(presentationMatch[1])',
    '    };',
    '  }',
    '',
    ...
    '}',
    '',
    'function resolveVariant(workflowId, variantId) {',
    '  const workflow = workflowList.find((entry) => entry.id === workflowId) ?? null;',
    '',
    '  if (!workflow) {',
    '    return {',
    ...
    '  }',
    '',
    '  const variant = workflow.variants?.find((entry) => entry.id === variantId) ?? null;',
    '',
    '  if (!variant) {',
    '    return {',
    '      component: null,',
    '      screen: null,',
    '      variant: null,',
    '      message: `Variant ${variantId} could not be found in workflow ${workflowId}.`',
    '    };',
    '  }',
    '',
    '  const resolvedScreen = resolveScreen(workflowId, variant.screenId);',
    '',
    '  if (!resolvedScreen.component || !resolvedScreen.screen) {',
    '    return {',
    '      component: null,',
    '      screen: null,',
    '      variant,',
    '      message: resolvedScreen.message',
    '    };',
    '  }',
    '',
    '  return {',
    '    component: resolvedScreen.component,',
    '    screen: resolvedScreen.screen,',
    '    variant,',
    '    message: ""',
    '  };',
    '}',
    ...
    '}',
    '',
    'async function renderPresentation(presentationId, version) {',
    '  try {',
    '    const response = await fetch(`./api/presentations/${encodeURIComponent(presentationId)}`);',
    '    if (!response.ok) {',
    '      renderMessage("Presentation not found", `No presentation with ID "${presentationId}".`);',
    '      return;',
    '    }',
    ...
    '    const vw = presentation.viewport?.width || 1280;',
    '    const vh = presentation.viewport?.height || 720;',
    '',
    '    const root = document.createElement("div");',
    '    root.style.cssText = "min-height:100vh;background:#fafafa;font-family:Inter,system-ui,-apple-system,sans-serif;";',
    '',
    '    const header = document.createElement("div");',
    '    header.style.cssText = "padding:1.25rem 1.5rem 0;";',
    '    const title = document.createElement("h1");',
    '    title.style.cssText = "margin:0;font-size:1.125rem;font-weight:600;color:#0a0a0a;";',
    '    title.textContent = presentation.title || "Presentation";',
    '    const subtitle = document.createElement("p");',
    '    subtitle.style.cssText = "margin:0.25rem 0 0;font-size:0.8125rem;color:#a3a3a3;";',
    '    subtitle.textContent = `${presentation.items.length} screen${presentation.items.length === 1 ? "" : "s"}`;',
    '    header.appendChild(title);',
    '    header.appendChild(subtitle);',
    '    root.appendChild(header);',
    '',
    '    const grid = document.createElement("div");',
    '    grid.style.cssText = "display:grid;grid-template-columns:repeat(auto-fill,minmax(min(100%,420px),1fr));gap:1.25rem;padding:1.25rem;";',
    '    root.appendChild(grid);',
    '    appTarget.appendChild(root);',
    '',
    ...
    '      cell.style.cssText = "display:flex;flex-direction:column;gap:0.375rem;";',
    '',
    '      const label = document.createElement("div");',
    '      label.style.cssText = "font-size:0.8125rem;font-weight:500;color:#525252;padding:0 0.125rem;";',
    '      label.textContent = item.label || "Screen";',
    '      cell.appendChild(label);',
    '',
    '      const frame = document.createElement("div");',
    '      frame.style.cssText = `position:relative;overflow:hidden;border-radius:0.5rem;border:1px solid #e5e5e5;background:#fff;aspect-ratio:${vw}/${vh};`;',
    '      cell.appendChild(frame);',
    '',
    '      const viewport = document.createElement("div");',
    '      viewport.style.cssText = `position:absolute;top:0;left:0;width:${vw}px;height:${vh}px;transform-origin:top left;`;',
    '      frame.appendChild(viewport);',
    '      grid.appendChild(cell);',
    '',
    '      try {',
    '        if (item.type === "screen") {',
    '          const resolved = resolveScreen(item.workflow, item.screen);',
    '          if (resolved.component) {',
    '            const inst = mount(resolved.component, { target: viewport, props: resolved.screen.props || {} });',
    '            mountedPresentationComponents.push(inst);',
    '          } else {',
    '            viewport.innerHTML = `<div style="padding:1rem;color:#a3a3a3;font-size:0.875rem">${escapeHtml(resolved.message)}</div>`;',
    '          }',
    '        } else if (item.type === "variant") {',
    '          const resolved = resolveVariant(item.workflow, item.variant);',
    '          if (resolved.component && resolved.screen && resolved.variant) {',
    '            const inst = mount(resolved.component, {',
    '              target: viewport,',
    '              props: { ...(resolved.screen.props || {}), ...(resolved.variant.props || {}) }',
    '            });',
    '            mountedPresentationComponents.push(inst);',
    '          } else {',
    '            viewport.innerHTML = `<div style="padding:1rem;color:#a3a3a3;font-size:0.875rem">${escapeHtml(resolved.message)}</div>`;',
    '          }',
    '        } else if (item.type === "render" && item.renderId) {',
    '          const renderModule = await import(/* @vite-ignore */ resolveRenderModuleImportUrl(item.renderId));',
    '          if (version !== renderVersion) return;',
    '          if (typeof renderModule.render === "function") {',
    '            const cleanup = await renderModule.render(viewport);',
    '            if (typeof cleanup === "function") mountedPresentationComponents.push({ $destroy: cleanup });',
    '          } else {',
    '            const Comp = renderModule.component || renderModule.default;',
    '            if (Comp) {',
    '              const inst = mount(Comp, { target: viewport, props: renderModule.props || {} });',
    '              mountedPresentationComponents.push(inst);',
    '            }',
    '          }',
    '        }',
    '      } catch (err) {',
    '        viewport.innerHTML = `<div style="padding:1rem;color:#a3a3a3;font-size:0.875rem">Failed to render: ${escapeHtml(formatError(err))}</div>`;',
    '      }',
    '    }',
    '',
    ...
    '        const scale = Math.min(fw / vw, 1);',
    '        const vp = entry.target.querySelector("div");',
    '        if (vp) vp.style.transform = `scale(${scale})`;',
    '        entry.target.style.height = `${vh * scale}px`;',
    '      }',
    '    });',
    '    grid.querySelectorAll("div[style*=aspect-ratio]").forEach((frame) => observer.observe(frame));',
    '  } catch (err) {',
    '    if (version !== renderVersion) return;',
    '    renderMessage("Presentation error", formatError(err));',
    '  }',
    '}',
    '',
    'function clearMount() {',
...
function getPreviewHtml(): string {
  // The inline script detects a reverse-proxy prefix (e.g. /proxy/5199/) and
  // injects an importmap so all absolute `/preview/…` module imports resolve
  // through the proxy instead of hitting the gateway origin directly.
  return [
    '<!doctype html><html lang="en"><head>',
    '<script>(function(){var b="/preview/";var i=location.pathname.indexOf(b);if(i>0){var m=document.createElement("script");m.type="importmap";m.textContent=JSON.stringify({imports:{["/preview/"]:location.pathname.slice(0,i)+b}});document.head.appendChild(m)}})()</script>',
    '<!--vite-->',
    '<meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Preview</title>',
